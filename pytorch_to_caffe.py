...
from models.py_utils._cpools import TopPoolFunction, BottomPoolFunction, LeftPoolFunction, RightPoolFunction
"""
...
def _up_sample(raw, input,size=None, scale_factor=None, mode='nearest', align_corners=None):
    if mode != "nearest" or align_corners != None:
        raise NotImplementedError("not implement F.interpolate totoaly")
    x = raw(input, size, scale_factor, mode)
    ...
    layer_name = log.add_layer(name='upsample')
    top_blobs = log.add_blobs([x], name='upsample_blob'.format(type))
    layer = caffe_net.Layer_param(name=layer_name, type='CPP',
                                  bottom=[log.blobs(input)], top=top_blobs)
    
    layer_param = "scale:{}; upsample_h: {}; upsample_w: {}".format(scale_factor, input.size(2) * scale_factor, input.size(3) * scale_factor)
    layer.cpp_param(layer_name='Upsample', layer_param=layer_param)
    log.cnet.add_layer(layer)
    return x


def _top_pool(raw, ctx, input):
    x = raw(ctx, input)

    layer_name = log.add_layer(name='top_pool')
    top_blobs = log.add_blobs([x], name='top_pool_blob'.format(type))
    layer = caffe_net.Layer_param(name=layer_name, type='CPP',
                                  bottom=[log.blobs(input)], top=top_blobs)
    layer.cpp_param(layer_name='TopCornerPoolLayer')
    log.cnet.add_layer(layer)
    return x


def _bottom_pool(raw, ctx, input):
    x = raw(ctx, input)

    layer_name = log.add_layer(name='bottom_pool')
    top_blobs = log.add_blobs([x], name='bottom_pool_blob'.format(type))
    layer = caffe_net.Layer_param(name=layer_name, type='CPP',
                                  bottom=[log.blobs(input)], top=top_blobs)
    
    layer.cpp_param(layer_name='BottomCornerPoolLayer')
    log.cnet.add_layer(layer)
    return x

def _left_pool(raw, ctx, input):
    x = raw(ctx, input)

    layer_name = log.add_layer(name='left_pool')
    top_blobs = log.add_blobs([x], name='left_pool_blob'.format(type))
    layer = caffe_net.Layer_param(name=layer_name, type='CPP',
                                  bottom=[log.blobs(input)], top=top_blobs)

    layer.cpp_param(layer_name='LeftCornerPoolLayer')
    log.cnet.add_layer(layer)
    return x

def _right_pool(raw, ctx, input):
    x = raw(ctx, input)

    layer_name = log.add_layer(name='right_pool')
    top_blobs = log.add_blobs([x], name='right_pool_blob'.format(type))
    layer = caffe_net.Layer_param(name=layer_name, type='CPP',
                                  bottom=[log.blobs(input)], top=top_blobs)

    layer.cpp_param(layer_name='RightCornerPoolLayer')
    log.cnet.add_layer(layer)
    return x
...
#1.0.0+
F.interpolate = Rp(F.interpolate,_up_sample)
#0.4.0
#F.upsample = Rp(F.upsample, _interpolate)

#cornernet
TopPoolFunction.forward = Rp(TopPoolFunction.forward, _top_pool)
BottomPoolFunction.forward = Rp(BottomPoolFunction.forward, _bottom_pool)
LeftPoolFunction.forward = Rp(LeftPoolFunction.forward, _left_pool)
RightPoolFunction.forward = Rp(RightPoolFunction.forward, _right_pool)
